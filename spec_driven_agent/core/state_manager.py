...
class WorkflowStateManager:
    ...
    async def transition_state(
        ...
        if not await self._validate_transition(current_state, new_phase):
            raise ValueError(f"Invalid transition from {current_state.current_phase} to {new_phase}")
    ...
    async def get_state_summary(self, workflow_id: UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "phase_completed_at": state.phase_completed_at.isoformat() if state.phase_completed_at else None,
    ...
    async def _validate_architecture_state(self, state: WorkflowState) -> Dict[str, Any]:
