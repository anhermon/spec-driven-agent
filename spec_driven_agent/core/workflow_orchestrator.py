...
class SpecDrivenWorkflowOrchestrator:
    ...
    async def transition_to_phase(
        ...
        async with self.workflow_lock:
            ...
            if not workflow:
                raise WorkflowOrchestrationError(f"Workflow {workflow_id} not found", workflow_id)
    ...
    async def get_workflow_status(self, workflow_id: UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_phases": len(WorkflowPhase.__dict__) - 1,  # Exclude internal attributes
    ...
    async def _notify_agents_of_phase_change(self, workflow_id: UUID, new_phase: WorkflowPhase) -> None:
        """Notify agents of phase change."""
