...
class WorkflowState(StatusModel):
    ...
    phase_data: Dict[str, Any] = Field(
        default_factory=dict, description="Phase-specific data and artifacts"
    )

    # Agent assignments
    active_agents: List[UUID] = Field(
        default_factory=list, description="Currently active agent IDs"
    )
    completed_agents: List[UUID] = Field(
        default_factory=list, description="Completed agent IDs"
    )

    # Task tracking
    pending_tasks: List[UUID] = Field(
        default_factory=list, description="Pending task IDs"
    )
    active_tasks: List[UUID] = Field(
        default_factory=list, description="Active task IDs"
    )
    completed_tasks: List[UUID] = Field(
        default_factory=list, description="Completed task IDs"
    )

    # Dependencies
    satisfied_dependencies: List[str] = Field(
        default_factory=list, description="Satisfied dependency IDs"
    )
    pending_dependencies: List[str] = Field(
        default_factory=list, description="Pending dependency IDs"
    )

    # User interactions
    user_approvals: List[str] = Field(
        default_factory=list, description="User approval checkpoints"
    )
    pending_decisions: List[str] = Field(
        default_factory=list, description="Pending user decisions"
    )
...
class WorkflowInstance(StatusModel):
    ...
    current_phase: WorkflowPhase = Field(
        default=WorkflowPhase.DISCOVERY, description="Current workflow phase"
    )
    ...
    phase_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="Phase transition history"
    )

    # State management
    state_id: Optional[UUID] = Field(None, description="Current state ID")
    state_history: List[UUID] = Field(
        default_factory=list, description="State history IDs"
    )

    # Agent coordination
    assigned_agents: List[UUID] = Field(
        default_factory=list, description="Assigned agent IDs"
    )
    agent_roles: Dict[UUID, str] = Field(
        default_factory=dict, description="Agent role assignments"
    )
...
class WorkflowTransition(StatusModel):
    ...
    dependencies_satisfied: bool = Field(
        ..., description="Whether all dependencies were satisfied"
    )
    ...
    transition_data: Dict[str, Any] = Field(
        default_factory=dict, description="Transition-specific data"
    )
