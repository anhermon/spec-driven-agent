...
class AgentContext(StatusModel):
    ...
    agent_state: Dict[str, Any] = Field(
        default_factory=dict, description="Agent state information"
    )

    # Project context reference
    project_context_id: Optional[UUID] = Field(
        None, description="Reference to project context"
    )
    ...
    active_conversations: List[str] = Field(
        default_factory=list, description="Active conversation IDs"
    )
...
class SymbolicReference(StatusModel):
    ...
    reference_type: str = Field(
        ..., description="Type of reference (e.g., 'api_spec', 'requirement')"
    )
    ...
    symbolic_properties: Dict[str, Any] = Field(
        default_factory=dict, description="Symbolic properties"
    )

    # Resolution
    resolved: bool = Field(default=False, description="Whether reference is resolved")
    resolved_at: Optional[datetime] = Field(
        None, description="When reference was resolved"
    )
    ...
    consistency_checks: List[str] = Field(
        default_factory=list, description="Applied consistency checks"
    )
...
class SymbolicData(StatusModel):
    ...
    # Data representation
    concrete_data: Optional[Any] = Field(
        None, description="Concrete data representation"
    )
    symbolic_representation: Dict[str, Any] = Field(
        default_factory=dict, description="Symbolic representation"
    )

    # Relationships
    parent_symbolic_id: Optional[str] = Field(
        None, description="Parent symbolic data ID"
    )
    child_symbolic_ids: List[str] = Field(
        default_factory=list, description="Child symbolic data IDs"
    )
    related_symbolic_ids: List[str] = Field(
        default_factory=list, description="Related symbolic data IDs"
    )
...
class ContextUpdate(StatusModel):
    ...
    processed: bool = Field(
        default=False, description="Whether update has been processed"
    )
    processed_at: Optional[datetime] = Field(
        None, description="When update was processed"
    )
    ...
    # Consistency
    consistency_impact: str = Field(
        default="unknown", description="Impact on context consistency"
    )
    consistency_checks: List[str] = Field(
        default_factory=list, description="Required consistency checks"
    )
...
class SpecDrivenContext(StatusModel):
    ...
    specifications: Dict[str, Any] = Field(
        default_factory=dict, description="API specifications"
    )
    architecture: Dict[str, Any] = Field(
        default_factory=dict, description="System architecture"
    )
    ...
    # Symbolic mechanisms
    symbolic_data: Dict[str, SymbolicData] = Field(
        default_factory=dict, description="Symbolic data"
    )
    symbolic_references: Dict[str, SymbolicReference] = Field(
        default_factory=dict, description="Symbolic references"
    )
    ...
    related_context_ids: List[UUID] = Field(
        default_factory=list, description="Related context IDs"
    )
    ...
    update_history: List[UUID] = Field(
        default_factory=list, description="Update history IDs"
    )

    # Consistency and validation
    consistency_status: str = Field(
        default="pending", description="Overall consistency status"
    )
    consistency_checks: List[str] = Field(
        default_factory=list, description="Applied consistency checks"
    )
    consistency_errors: List[str] = Field(
        default_factory=list, description="Consistency errors"
    )
    ...
    last_consistency_check: Optional[datetime] = Field(
        None, description="Last consistency check time"
    )
    consistency_check_duration: Optional[float] = Field(
        None, description="Consistency check duration"
    )
...
class ContextConsistencyValidator(StatusModel):
    ...
    # Results
    last_validation_result: Optional[Dict[str, Any]] = Field(
        None, description="Last validation result"
    )
    ...
    priority: int = Field(
        default=1, description="Validator priority (lower = higher priority)"
    )
