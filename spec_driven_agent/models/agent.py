...
class AgentCapability(StatusModel):
    ...
    constraints: List[str] = Field(default_factory=list, description="Capability constraints")
...
class AgentContext(StatusModel):
    ...
    knowledge_base: Dict[str, Any] = Field(default_factory=dict, description="Agent knowledge base")
    
    # Communication
    recent_messages: List[str] = Field(default_factory=list, description="Recent message IDs")
    communication_history: List[str] = Field(default_factory=list, description="Communication history")
    
    # State
    current_state: str = Field(default="idle", description="Current agent state")
    state_history: List[Dict[str, Any]] = Field(default_factory=list, description="State history")
...
class AgentMessage(StatusModel):
    ...
    # Routing
    routing_path: List[UUID] = Field(default_factory=list, description="Message routing path")
...
class Agent(StatusModel):
    ...
    # Communication
    communication_protocol: str = Field(default="a2a", description="Communication protocol")
    ...
    performance_metrics: Dict[str, Any] = Field(default_factory=dict, description="Performance metrics")
    task_completion_rate: Optional[float] = Field(None, description="Task completion rate")
    ...
    subordinate_ids: List[UUID] = Field(default_factory=list, description="Subordinate agent IDs")
    collaborator_ids: List[UUID] = Field(default_factory=list, description="Collaborator agent IDs")
    
    # History
    task_history: List[UUID] = Field(default_factory=list, description="Task history")
    workflow_history: List[UUID] = Field(default_factory=list, description="Workflow history")
    message_history: List[str] = Field(default_factory=list, description="Message history")
