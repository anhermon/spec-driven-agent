...
class Project(StatusModel):
    ...
    business_case: Optional[str] = Field(None, description="Business case and justification")
    
    # Stakeholder information
    stakeholders: List[str] = Field(default_factory=list, description="List of stakeholder names")
    product_owner: Optional[str] = Field(None, description="Product owner name")
    
    # Technical constraints
    technical_constraints: List[str] = Field(default_factory=list, description="Technical constraints and requirements")
    ...
    # Workflow tracking
    current_phase: str = Field(default=ProjectStatus.DRAFT, description="Current workflow phase")
    workflow_instance_id: Optional[UUID] = Field(None, description="Associated workflow instance")
    
    # Context and artifacts
    context_id: Optional[UUID] = Field(None, description="Associated context ID")
    artifact_ids: List[UUID] = Field(default_factory=list, description="List of artifact IDs")
